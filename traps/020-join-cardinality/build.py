"""Second probe of tabular-join: the join that multiplies the land.

Ten parcels of 5000 m2 — 50000 m2 of land. The owners table has thirteen rows
because three parcels are jointly held by two people each. Join and sum and
the three shared parcels are counted twice: 65000 m2, which is 30% more land
than exists and still an ordinary figure for a cadastral sheet.
"""
...
def main(destination: Path) -> int:
    ...
    owners = [("P-1", "Rossi"), ("P-1", "Bianchi"), ("P-2", "Verdi"), ("P-2", "Neri"),
              ("P-3", "Gialli"), ("P-3", "Blu")]
    owners += [(p, "Sole") for p in parcels[3:]]
    lines = ["parcel_id,owner"] + [f"{p},{o}" for p, o in owners]
